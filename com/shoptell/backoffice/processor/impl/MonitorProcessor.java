...
@Named
public class MonitorProcessor extends DataProcessor {
	...
	/* 6 */+ "BRONZE|CARBON|CHROME|COFFEE|COPPER|FERVOR|GOLDEN|MARBLE|ORANGE|PEBBLE|PURPLE|SILVER|YELLOW|"
	...
	private ReviewedProductInfoDTO processorUtil(HomeProductInfoDTO prod) {
		...
		if(StringUtils.isNotBlank(prod.getProductBrand())){
			...
			monitorProperties.setProduct_brand(prod.getProductBrand());
		}
		
		retrieveMemory(prod, monitorProperties); /*
												 * internal, ram and graphic
												 * memory; hard disk memory type
												 */
		
		retrieveColor(prod, monitorProperties); /* BLACK, WHITE */

		retrieveScreenSize(prod, monitorProperties); /* 17.3" */

		retrieveGeneration(prod, monitorProperties); /* 3RD GEN */

		retrieveRAMType(prod, monitorProperties); /* DDR3 DDR4 */
		
		retrieveProcessor(prod, monitorProperties); /* processor like i5 */
		
		retrieveOS(prod, monitorProperties); /* operating system */

		prod.setDescription(desc);

		if (StringUtils.isNotBlank(monitorProperties.getInternal_memory())) {
			return populateDto(prod, monitorProperties);
		}
		else {
			return null;
		}
	...
	private String retrieveColorUtil(String target, MonitorProperties monitorProperties) {
		String color = null;
		...
		if (StringUtils.isNotBlank(color)) {
			monitorProperties.setColor(color.toUpperCase());
		}

		return target;
	}

	private void retrieveColor(HomeProductInfoDTO prod, MonitorProperties monitorProperties) {

		if (StringUtils.isBlank(monitorProperties.getColor())) {
			String color = prod.getColor();
			if (StringUtils.isNotBlank(color)) {
				color = color.replaceAll("\\/|\\(|\\)|\\-|\\,|\\+", " ").trim();
				color = color.replaceAll("\\s+", " ").trim();
				retrieveColorUtil(color, monitorProperties);
			}
		}

		if (StringUtils.isBlank(monitorProperties.getColor())) {
			if (StringUtils.isNotBlank(prod.getTitle())) {
				prod.setTitle(retrieveColorUtil(prod.getTitle(), monitorProperties));
			}
		}

		if (StringUtils.isBlank(monitorProperties.getColor())) {
			if (StringUtils.isNotBlank(prod.getDescription())) {
				prod.setDescription(retrieveColorUtil(prod.getDescription(), monitorProperties));
			}
		}
	...
	private String retrieveMemoryUtil(String target, String brand, MonitorProperties monitorProperties, boolean fromSize) {
		List<String> allMatches = new LinkedList<String>();
		int[] sizeInNum = null;

		String[] size_as_string = new String[3]; // 0: hdd; 1: ram; 2: graphics
		String size_in_ssd = StringUtils.EMPTY, size_in_sshd = StringUtils.EMPTY;
		...
		target = target.replaceAll("FLASH", "SSD").replaceAll("\\s+TB", "TB").replaceAll("\\s+GB", "GB").replaceAll("\\s+MB", "MB")
				.replaceAll("\\s+SSD", "SSD").replaceAll("\\s+SSHD", "SSHD");
		...
		if (sizeInNum_length > 2) {
			...
			if (apple && StringUtils.isBlank(size_in_ssd)) {
				...
				monitorProperties.setHard_disk_type("HDD");
			}
		...
		// add ssd or sshd to hdd
		if (size_as_string != null) {
			if (StringUtils.isNotBlank(size_in_ssd) && !apple) {
				size_as_string[0] = (StringUtils.isNotBlank(size_as_string[0]) ? size_as_string[0] + " + " : "") + size_in_ssd;
				monitorProperties.setHard_disk_type((StringUtils.isNotBlank(monitorProperties.getHard_disk_type()) ? monitorProperties.getHard_disk_type() + " "
						: "") + "SSD");
			}
			if (StringUtils.isNotBlank(size_in_sshd) && !apple) {
				size_as_string[0] = (StringUtils.isNotBlank(size_as_string[0]) ? size_as_string[0] + " + " : "") + size_in_sshd;
				monitorProperties.setHard_disk_type((StringUtils.isNotBlank(monitorProperties.getHard_disk_type()) ? monitorProperties.getHard_disk_type() + " "
						: "") + "SSHD");
			}

			if (StringUtils.isBlank(monitorProperties.getInternal_memory()) && StringUtils.isNotBlank(size_as_string[0])) {
				monitorProperties.setInternal_memory(size_as_string[0].toUpperCase());
			}
			if (StringUtils.isBlank(monitorProperties.getRAM_memory()) && StringUtils.isNotBlank(size_as_string[1])) {
				monitorProperties.setRAM_memory(size_as_string[1].toUpperCase());
			}
			if (StringUtils.isBlank(monitorProperties.getGraphic_memory()) && StringUtils.isNotBlank(size_as_string[2])) {
				monitorProperties.setGraphic_memory(size_as_string[2].toUpperCase());
			}
		}
	...
	private void retrieveMemory(HomeProductInfoDTO prod, MonitorProperties monitorProperties) {
		if (StringUtils.isBlank(monitorProperties.getInternal_memory()) || StringUtils.isBlank(monitorProperties.getRAM_memory())
				|| StringUtils.isBlank(monitorProperties.getGraphic_memory())) {
			String size = prod.getSize();
			if (StringUtils.isNotBlank(size)) {
				size = size.replaceAll("\\/|\\(|\\)|\\-|\\,|\\+", " ").trim();
				size = size.replaceAll("\\s+", " ").trim();
				retrieveMemoryUtil(size, prod.getProductBrand(), monitorProperties, true);
			}
		}
		...
		if (StringUtils.isBlank(monitorProperties.getInternal_memory()) || StringUtils.isBlank(monitorProperties.getRAM_memory())
				|| StringUtils.isBlank(monitorProperties.getGraphic_memory())) {
			if (StringUtils.isNotBlank(prod.getTitle())) {
				prod.setTitle(retrieveMemoryUtil(prod.getTitle(), prod.getProductBrand(), monitorProperties, false));
			}
		}

		/* description messes with graphic memory */
		/*
		 * if (StringUtils.isBlank(monitorProperties.getInternal_memory()) ||
		 * StringUtils.isBlank(monitorProperties.getRAM_memory()) ||
		 * StringUtils.isBlank(monitorProperties.getGraphic_memory())) { if
		 * (StringUtils.isNotBlank(prod.getDescription())) {
		 * prod.setDescription(retrieveMemoryUtil(prod.getDescription(),
		 * prod.getProductBrand(), monitorProperties)); } }
		 */
	}

	private String retrieveOSUtil(String target, String brand, MonitorProperties monitorProperties) {
		...
		if (StringUtils.isNotBlank(os)) {
			monitorProperties.setOperatingSystem(os.toUpperCase());
		}

		return target;
	}

	private void retrieveOS(HomeProductInfoDTO prod, MonitorProperties monitorProperties) {

		if (StringUtils.isBlank(monitorProperties.getOperatingSystem())) {
			if (StringUtils.isNotBlank(prod.getTitle())) {
				prod.setTitle(retrieveOSUtil(prod.getTitle(), prod.getProductBrand(), monitorProperties));
			}
		}

		if (StringUtils.isBlank(monitorProperties.getOperatingSystem())) {
			if (StringUtils.isNotBlank(prod.getDescription())) {
				prod.setDescription(retrieveOSUtil(prod.getDescription(), prod.getProductBrand(), monitorProperties));
			}
		}
	...
	private String retrieveProcessorUtil(String target, String regex, MonitorProperties monitorProperties) {
		...
		if (StringUtils.isNotBlank(processor)) {
			monitorProperties.setProcessor(processor.toUpperCase());
		}

		return target;
	}

	private void retrieveProcessor(HomeProductInfoDTO prod, MonitorProperties monitorProperties) {
		if (StringUtils.isBlank(monitorProperties.getProcessor()) && StringUtils.isNotBlank(prod.getTitle())) {
			prod.setTitle(retrieveProcessorUtil(prod.getTitle(), AMD_PROCESSOR_REGEX, monitorProperties));
			if (StringUtils.isNotBlank(monitorProperties.getProcessor())) {
				monitorProperties.setProcessor("AMD " + monitorProperties.getProcessor().toUpperCase());
			}
		}

		if (StringUtils.isBlank(monitorProperties.getProcessor()) && StringUtils.isNotBlank(prod.getTitle())) {
			prod.setTitle(retrieveProcessorUtil(prod.getTitle(), INTEL_PROCESSOR_REGEX, monitorProperties));
			if (StringUtils.isNotBlank(monitorProperties.getProcessor())) {
				monitorProperties.setProcessor("INTEL " + monitorProperties.getProcessor().toUpperCase());
			}
		}

		if (StringUtils.isBlank(monitorProperties.getProcessor()) && StringUtils.isNotBlank(prod.getDescription())) {
			prod.setDescription(retrieveProcessorUtil(prod.getDescription(), AMD_PROCESSOR_REGEX, monitorProperties));
			if (StringUtils.isNotBlank(monitorProperties.getProcessor())) {
				monitorProperties.setProcessor("AMD " + monitorProperties.getProcessor().toUpperCase());
			}
		}
		...
		if (StringUtils.isBlank(monitorProperties.getProcessor()) && StringUtils.isNotBlank(prod.getDescription())) {
			prod.setDescription(retrieveProcessorUtil(prod.getDescription(), INTEL_PROCESSOR_REGEX, monitorProperties));
			if (StringUtils.isNotBlank(monitorProperties.getProcessor())) {
				monitorProperties.setProcessor("INTEL " + monitorProperties.getProcessor().toUpperCase());
			}
		}
	...
	private String retrieveScreenSizeUtil(String target, MonitorProperties monitorProperties) {
		Pattern cm_pattern = Pattern.compile("\\d+\\.?\\d*\\s*CM");
		Pattern inch_pattern = Pattern.compile("\\d+\\.?\\d*\\s*(INCH(ES)?|\")");
		...
		if (StringUtils.isNotBlank(screenSize)) {
			monitorProperties.setScreenSize(screenSize.toUpperCase());
		}

		return target;
	}

	private void retrieveScreenSize(HomeProductInfoDTO prod, MonitorProperties monitorProperties) {

		/*if (StringUtils.isBlank(monitorProperties.getScreenSize())) {
			String size = prod.getSize();
			if (StringUtils.isNotBlank(prod.getSize())) {
				size = size.replaceAll("\\/|\\(|\\)|\\-|\\,|\\+", " ").trim();
				size = size.replaceAll("\\s+", " ").trim();
				retrieveScreenSizeUtil(size, monitorProperties);
			}
		}*/

		if (StringUtils.isBlank(monitorProperties.getScreenSize())) {
			if (StringUtils.isNotBlank(prod.getTitle())) {
				prod.setTitle(retrieveScreenSizeUtil(prod.getTitle(), monitorProperties));
			}
		}

		if (StringUtils.isBlank(monitorProperties.getScreenSize())) {
			if (StringUtils.isNotBlank(prod.getDescription())) {
				prod.setDescription(retrieveScreenSizeUtil(prod.getDescription(), monitorProperties));
			}
		}

	}

	private String retrieveGenerationUtil(String target, MonitorProperties monitorProperties) {
		...
		if (StringUtils.isNotBlank(generation)) {
			monitorProperties.setGeneration(generation.toUpperCase());
		}
		return target;
	}

	private void retrieveGeneration(HomeProductInfoDTO prod, MonitorProperties monitorProperties) {
		if (StringUtils.isBlank(monitorProperties.getGeneration())) {
			if (StringUtils.isNotBlank(prod.getTitle())) {
				prod.setTitle(retrieveGenerationUtil(prod.getTitle(), monitorProperties));
			}
		}

		if (StringUtils.isBlank(monitorProperties.getGeneration())) {
			if (StringUtils.isNotBlank(prod.getDescription())) {
				prod.setDescription(retrieveGenerationUtil(prod.getDescription(), monitorProperties));
			}
		}
	}

	private String retrieveRAMTypeUtil(String target, MonitorProperties monitorProperties) {
		...
		while (matcher.find()) {
			...
			if (ram_type.startsWith("DD")) {
				...
				monitorProperties.setRAM_type(ram_type.toUpperCase());
				break;
			}
		}
	...
	private void retrieveRAMType(HomeProductInfoDTO prod, MonitorProperties monitorProperties) {
		if (StringUtils.isBlank(monitorProperties.getRAM_type())) {
			if (StringUtils.isNotBlank(prod.getTitle())) {
				prod.setTitle(retrieveRAMTypeUtil(prod.getTitle(), monitorProperties));
			}
		}

		if (StringUtils.isBlank(monitorProperties.getRAM_type())) {
			if (StringUtils.isNotBlank(prod.getDescription())) {
				prod.setDescription(retrieveRAMTypeUtil(prod.getDescription(), monitorProperties));
			}
		}
	}

	private ReviewedProductInfoDTO populateDto(HomeProductInfoDTO prod, MonitorProperties monitorProperties) {
		...
		String color =  monitorProperties.getColor();
		if (StringUtils.isNotBlank(color)){
		...
		info.getProperties().put("INTERNAL MEMORY",
				StringUtils.isNotBlank(monitorProperties.getInternal_memory()) ? monitorProperties.getInternal_memory() : "NULL");
		info.getProperties().put("RAM", StringUtils.isNotBlank(monitorProperties.getRAM_memory()) ? monitorProperties.getRAM_memory() : "NULL");
		info.getProperties().put("RAM TYPE", StringUtils.isNotBlank(monitorProperties.getRAM_type()) ? monitorProperties.getRAM_type() : "NULL");
		info.getProperties().put("MEMORY TYPE", StringUtils.isNotBlank(monitorProperties.getHard_disk_type()) ? monitorProperties.getHard_disk_type() : "NULL");
		info.getProperties()
				.put("GRAPHIC MEMORY", StringUtils.isNotBlank(monitorProperties.getGraphic_memory()) ? monitorProperties.getGraphic_memory() : "NULL");
		info.getProperties().put("SCREEN SIZE", StringUtils.isNotBlank(monitorProperties.getScreenSize()) ? monitorProperties.getScreenSize() : "NULL");
		info.getProperties().put("O/S", StringUtils.isNotBlank(monitorProperties.getOperatingSystem()) ? monitorProperties.getOperatingSystem() : "NULL");
		info.getProperties().put("PROCESSOR", StringUtils.isNotBlank(monitorProperties.getProcessor()) ? monitorProperties.getProcessor() : "NULL");
		info.getProperties().put("GENERATION", StringUtils.isNotBlank(monitorProperties.getGeneration()) ? monitorProperties.getGeneration() : "NULL");
	...
	/*public static void main(String[] args) {
		...
		_hpi.setTitle("DELL INSPIRON 3542 NOTEBOOK (4TH GEN INTEL CORE I3- 4GB RAM- 1TB HDD- 15.6 INCHES- UBUNTU) (SILVER) (3542341TBISU)");
		_hpi.setSize("4GB");
		_hpi.setProductBrand("DELL");
		_hpi.setOriginalTitle("DELL INSPIRON 3542 NOTEBOOK (4TH GEN INTEL CORE I3- 4GB RAM- 1TB HDD- 15.6 INCHES- UBUNTU) (SILVER) (3542341TBISU)");
